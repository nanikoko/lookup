...
%>
	...
	<section>
		...
		<div class="mainContent">
		  ...
		  <!-- 내용 작성 -->
		  <div class="subbox">
		      <form action="<%=request.getContextPath()%>/qnaUpdate.do" method="post">
				<input type="hidden" name="qna_no" value="<%=vo.getQna_no()%>">  
	  	      
	  	      <div id="titleInputbox">
 	  	        <label for="titleInput"> 제 목 </label>
		        <input type="text" name="title" style="width:50%" value="<%=vo.getTitle()%>">
		        <br><br>
 	  	        <label for="titleInput"> 작성자 </label>
		        <input type="text" name="mem_id" style="width:50%" value="<%=vo.getMem_id()%>">
		      </div>
